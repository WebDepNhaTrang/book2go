...
@section('content')
    <h3 class="form-title font-green">{{ trans('core.acl::auth.forgot_password.title') }}</h3>
        ...
        <div class="form-group">
            <label class="control-label visible-ie8 visible-ie9">{{ trans('core.acl::auth.login.email') }}</label>
            {!! Form::text('email', old('email'), ['class' => 'form-control placeholder-no-fix', 'placeholder' => trans('core.acl::auth.login.placeholder.email')]) !!}
        </div>
        <div class="form-group">
            <button type="submit" class="btn green uppercase">{{ trans('core.acl::auth.forgot_password.submit') }}</button>
        </div>
        {!! Form::close() !!}
