...
class Handler extends ExceptionHandler
{
        ...
        if ($ex instanceof AuthorizationException) {
            if ($request->is(config('core.base.general.admin_dir') . '/*') || $request->is(config('core.base.general.admin_dir'))) {
                if ($request->ajax() || $request->wantsJson()) {
                    return $response->setError(true)->setMessage(trans('core.acl::permissions.access_denied_message'));
                }
                return response()->view('core.base::errors.401', [], 403);
            }
        ...
        if ($this->isHttpException($ex)) {
            ...
            if (in_array($code, [401, 403])) {
                if ($request->is(config('core.base.general.admin_dir') . '/*') || $request->is(config('core.base.general.admin_dir'))) {
                    ...
                    return response()->view('core.base::errors.401', [], 401);
                }
                admin_bar()->setIsDisplay(false);
                return $response->setError(true)->setMessage(trans('core.acl::permissions.access_denied_message'));
            }
            ...
            if ($code == 404) {
                if ($request->is(config('core.base.general.admin_dir') . '/*') || $request->is(config('core.base.general.admin_dir'))) {
                    return response()->view('core.base::errors.404', [], 404);
                }
            ...
            if (in_array($code, [500, 503])) {
                if ($request->is(config('core.base.general.admin_dir') . '/*') || $request->is(config('core.base.general.admin_dir'))) {
                    return response()->view('core.base::errors.500', [], 500);
                }
                $theme = Theme::uses(setting('theme'))->layout(setting('layout', 'default'));
    ...
    protected function unauthenticated($request, AuthenticationException $exception)
    {
        return $request->expectsJson()
            ...
            : redirect()->guest(route('access.login'));
    }
}
