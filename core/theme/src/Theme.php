...
class Theme implements ThemeContract
{
     ...
     * @author Sang Nguyen
     */
    public function checkViewExists($view, $return = false)
    {
        if ($return) {
        ...
        if (!$this->view->exists($view)) {
            if (app()->environment() == 'local') {
                dd('Theme is not support this view, please create file "/public/themes/' . str_replace($this->getThemeNamespace(), $this->getThemeName(), str_replace('::', '/', str_replace('.', '/', $view))) . '.blade.php" to render this page!');
            } else {
                abort(404);
            }
        }
