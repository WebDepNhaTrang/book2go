...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function registerMenuOptions()
    {
        ...
        $posts = Menu::generateSelect([
            'model' => app(PostInterface::class)->getModel(),
            'screen' => POST_MODULE_SCREEN_NAME,
            'theme' => false,
            'view' => 'plugins.blog::posts.partials.select',
            'options' => [
                'class' => 'list-item',
            ]
        ]);
        echo view('plugins.blog::posts.partials.menu-options', compact('posts'));
    }
            ...
            switch ($slug->reference) {
                case POST_MODULE_SCREEN_NAME:
                    ...
                    if (!empty($post)) {
                        ...
                        $data = [
                            'template' => $post->template ? $post->template : null,
                            'view' => $post->template ? $post->template :'post',
                            'data' => compact('post'),
                        ];
                    }
                    break;
                case CATEGORY_MODULE_SCREEN_NAME:
                    ...
                    if (!empty($category)) {
                        ...
                        $posts = app(PostInterface::class)->getByCategory($category->id, 12);
                        ...
                        return [
                            'template' => $category->template ? $category->template : null,
                            'view' => $category->template ? $category->template :'category',
