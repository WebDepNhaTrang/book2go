...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
     ...
     * @author Anh ngo
     */
    public function getNextPost($order, $category_id, $id)
    {
        $data = $this->model->where('posts.status', '=', 1)
            ->join('post_category', 'post_category.post_id', '=', 'posts.id')
            ->join('categories', 'post_category.category_id', '=', 'categories.id')
            ->where('post_category.category_id', '=' , $category_id)
            ->where('posts.order', '>=', $order)
            ->where('posts.id', '!=', $id)
            ->select('posts.*')
            ->distinct()
            ->orderBy('order', 'asc')
            ->orderBy('created_at', 'asc');
        $data = apply_filters(BASE_FILTER_BEFORE_GET_FRONT_PAGE_ITEM, $data, $this->model, POST_MODULE_SCREEN_NAME)->first();
        $this->resetModel();
        return $data;
    }
     ...
     * @author Anh ngo
     */
    public function getPreviousPost($order, $category_id, $id)
    {
        $data = $this->model->where('posts.status', '=', 1)
            ->join('post_category', 'post_category.post_id', '=', 'posts.id')
            ->join('categories', 'post_category.category_id', '=', 'categories.id')
