...
class EventsTable extends TableAbstract
{
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                return anchor_link(route('events.edit', $item->id), $item->name);
            })
    ...
    {
        $model = app(EventsInterface::class)->getModel();
        /**
        * @var \Eloquent $model
        */
        $query = $model->select(['events.id', 'events.name', 'events.created_at', 'events.status']);
    ...
    {
        return EVENTS_MODULE_SCREEN_NAME;
    }
}
