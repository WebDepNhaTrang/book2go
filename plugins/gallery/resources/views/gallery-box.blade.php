        ...
        <div class="row">
            @if (!empty($value))
                @foreach ($value as $key => $item)
                    <div class="col-md-3 col-sm-4 col-xs-4 photo-gallery-item" data-id="{{ $key }}">
        ...
        $('.btn_select_gallery').rvMedia({
            onSelectFiles: function (files, $el) {
                ...
                $.each(files, function (index, file) {
                    ...
                    $('.list-photos-gallery .row').append('<div class="col-md-3 col-sm-4 col-xs-6 photo-gallery-item" data-id="' + (last_index + index) + '"><div class="gallery_image_wrapper"><img src="' + file.thumb + '" /></div></div>');
                });
