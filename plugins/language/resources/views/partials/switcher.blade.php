...
{{--     <ul class="language_bar_list {{ array_get($options, 'class') }}">
    ...
    <li class="selectLanguage">
      <div class="dropdown Language">
        <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">{{ Language::getCurrentLocale() }}
          <span class="caret"></span>
        </button>
        <ul class="dropdown-menu">
            @foreach (Language::getSupportedLocales() as $localeCode => $properties)
                <li @if ($localeCode == Language::getCurrentLocale()) class="active-language" @endif>
                  <a rel="alternate" hreflang="{{ $localeCode }}" href="{{ $show_related ? Language::getLocalizedURL($localeCode) : url($localeCode) }}"><span>{{ $localeCode }}</span></a>
                </li>
            @endforeach
        </ul>
      </div>
